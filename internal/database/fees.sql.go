...
const getFeeStructurePerTermForStudent = `-- name: GetFeeStructurePerTermForStudent :one
SELECT 
    fs.fee_structure_id,
    fs.term_id, 
    fs.class_id
...
type GetFeeStructurePerTermForStudentParams struct {
	TermID    uuid.UUID `json:"term_id"`
	StudentID uuid.UUID `json:"student_id"`
}

type GetFeeStructurePerTermForStudentRow struct {
	FeeStructureID uuid.UUID `json:"fee_structure_id"`
	TermID         uuid.UUID `json:"term_id"`
	ClassID        uuid.UUID `json:"class_id"`
}

func (q *Queries) GetFeeStructurePerTermForStudent(ctx context.Context, arg GetFeeStructurePerTermForStudentParams) (GetFeeStructurePerTermForStudentRow, error) {
	row := q.db.QueryRow(ctx, getFeeStructurePerTermForStudent, arg.TermID, arg.StudentID)
	var i GetFeeStructurePerTermForStudentRow
	err := row.Scan(&i.FeeStructureID, &i.TermID, &i.ClassID)
	return i, err
}
...
const getStudentPreviousFeeRecord = `-- name: GetStudentPreviousFeeRecord :one
SELECT
    fees.fees_id,
    students.student_id,
    fees.paid,
...
type GetStudentPreviousFeeRecordParams struct {
	TermID    uuid.UUID `json:"term_id"`
	StudentID uuid.UUID `json:"student_id"`
}

type GetStudentPreviousFeeRecordRow struct {
	FeesID        uuid.UUID      `json:"fees_id"`
	StudentID     uuid.UUID      `json:"student_id"`
	Paid          pgtype.Numeric `json:"paid"`
	Arrears       pgtype.Numeric `json:"arrears"`
...
func (q *Queries) GetStudentPreviousFeeRecord(ctx context.Context, arg GetStudentPreviousFeeRecordParams) (GetStudentPreviousFeeRecordRow, error) {
	row := q.db.QueryRow(ctx, getStudentPreviousFeeRecord, arg.TermID, arg.StudentID)
	var i GetStudentPreviousFeeRecordRow
	err := row.Scan(
		&i.FeesID,
		&i.StudentID,
		&i.Paid,
