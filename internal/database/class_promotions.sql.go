...
updated_student_classes AS (
    ...
    RETURNING sc.student_class_id, sc.student_id, sc.previous_class_id, sc.class_id, sc.term_id
),
updated_students AS (
    UPDATE students s
    SET
    ...
    RETURNING s.student_id, s.student_no, s.academic_year_id, s.last_name, s.middle_name, s.first_name, s.gender, s.date_of_birth, s.status, s.promoted, s.graduated, s.suspended
)
SELECT
...
func (q *Queries) UndoPromoteStudents(ctx context.Context, dollar_1 uuid.UUID) error {
	_, err := q.db.Exec(ctx, undoPromoteStudents, dollar_1)
	return err
}
