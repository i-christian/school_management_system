...
SELECT
    st_cl.student_id,
    st.student_no,
    st.first_name || ' ' || COALESCE(st.middle_name || ' ', '') || st.last_name AS student_name,
    s.subject_id,
    ...
    t.name AS term_name,
    t.academic_year_id
FROM student_classes st_cl
JOIN students st 
  ON st_cl.student_id = st.student_id
JOIN assignments a 
  ON st_cl.class_id = a.class_id
JOIN subjects s 
